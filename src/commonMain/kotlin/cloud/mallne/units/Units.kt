...
import cloud.mallne.units.NumericalExtensions.removeTrailingZeros
import cloud.mallne.units.NumericalExtensions.round
...
abstract class Units(
    ...
    val databasePrimitive: String? = null
) {
    /**
    ...
    companion object {
        @Serializable
        enum class SISystems(val units: List<IUnitStore<*>>) {
            ANGLE(Angle.Companion.UnitStore.entries),
            AREA(Area.Companion.UnitStore.entries),
            BINARY_SIZE(BinarySize.Companion.UnitStore.entries),
            GRAPHICS_LENGTH(GraphicsLength.Companion.UnitStore.entries),
            LENGTH(Length.Companion.UnitStore.entries),
            MASS(Mass.Companion.UnitStore.entries),
            PRESSURE(Pressure.Companion.UnitStore.entries),
            PROBABILITY(Probability.Companion.UnitStore.entries),
            TEMPERATURE(Temperature.Companion.UnitStore.entries),
            TIME(Time.Companion.UnitStore.entries),
            VOLUME(Volume.Companion.UnitStore.entries),
            WOOD_VOLUME(WoodVolume.Companion.UnitStore.entries),
            WORK(Work.Companion.UnitStore.entries),
        }
    }
}
 ...
 */
class UnitsProduct<A : Units, B : Units>(val first: A, val second: B) :
    Units(if (first == second) "($first)^2" else "$first$second", first.ratio * second.ratio)
 ...
 */
class UnitsRatio<A : Units, B : Units>(val numerator: A, val denominator: B) :
    Units("$numerator/$denominator", numerator.ratio / denominator.ratio) {
...
class Measure<T : Units>(val amount: Double, val units: T) : Comparable<Measure<T>> {
    ...
    infix fun <A : T> `as`(other: A): Measure<T> =
        if (units == other) this else Measure(this `in` other, other)

    /**
     * Gets the value of the Measure in the given unit.
     */
    infix fun <A : T> `in`(other: A): Double =
        if (units == other) amount else amount * (units.ratio / other.ratio)

    /**
     * Add another compatible quantity to this one
     */
    operator fun plus(other: Measure<T>): Measure<T> =
        minOf(units, other.units).let { Measure((this `in` it) + (other `in` it), it) }
    ...
    override fun compareTo(other: Measure<T>): Int =
        (this `as` other.units).amount.compareTo(other.amount)
...
//        if (this.amount == 0.0 && other.amount == 0.0) return true TODO: Should this be true?
    ...
    fun toString(decimals: Int): String =
        "${amount.round(decimals).removeTrailingZeros()}${units.measureSuffix()}"
}

fun Measure<*>?.safeString(ifNull: String = "", decimals: Int = 2): String =
    this?.toString(decimals) ?: ifNull

fun <T : Units> Measure<T>.autoConvert(
    baseUnit: T,
    startingBest: T,
    convertables: List<T>
): Measure<T> {
    // this should automatically convert to the best unit using the Ratio
    val asM = this `as` baseUnit
    var currentBest = startingBest
    val sorted = convertables.sortedBy { it.ratio }
    sorted.forEach {
        if (asM.amount >= it.ratio) {
            currentBest = it
        }
    }
    return this `as` currentBest
}

// region ================ Units * Units Math ============================

/** A * B              */
operator fun <A : Units, B : Units> A.times(other: B): UnitsProduct<A, B> =
    UnitsProduct(this, other)

@JvmName("times7")
operator fun <A : Units> A.times(other: InverseUnits<A>): Double = ratio / other.ratio
...
operator fun <A : Units> A.times(other: InverseUnits<Square<A>>): Measure<InverseUnits<A>> =
    ratio / other.unit.first.ratio / other.unit.first

/** A * (B / A)        */
operator fun <A : Units, B : Units> A.times(other: UnitsRatio<B, A>): Measure<B> =
    ratio / other.denominator.ratio * other.numerator

/** (A / B) * B        */
@JvmName("times1")
operator fun <A : Units, B : Units> UnitsRatio<A, B>.times(other: B): Measure<A> =
    other.ratio / denominator.ratio * numerator

/** (A / (B * C)) * B  */
@JvmName("times2")
operator fun <A : Units, B : Units, C : Units> UnitsRatio<A, UnitsProduct<B, C>>.times(other: B): Measure<UnitsRatio<A, C>> =
    other.ratio / denominator.first.ratio * (numerator / denominator.second)

/** (A / (B * C)) * D) */
@JvmName("times3")
operator fun <A : Units, B : Units, C : Units, D : Units> UnitsRatio<A, UnitsProduct<B, C>>.times(
    other: D
): UnitsRatio<UnitsProduct<A, D>, UnitsProduct<B, C>> = numerator * other / denominator
...
@JvmName("times8")
operator fun <A : Units> InverseUnits<A>.times(other: A): Double = ratio * other.ratio

@JvmName("times9")
operator fun <A : Units, B : Units> InverseUnits<A>.times(other: B): UnitsRatio<B, A> = other * this
...
operator fun <A : Units, B : Units> UnitsProduct<A, B>.div(other: UnitsProduct<B, B>): Measure<UnitsRatio<A, B>> =
    second.ratio / other.second.ratio * (first / other.first)
...
operator fun <A : Units, B : Units> UnitsRatio<A, B>.div(other: A): Measure<InverseUnits<B>> =
    (numerator.ratio / other.ratio) / denominator
...
operator fun <A : Units, B : Units> UnitsRatio<UnitsProduct<A, A>, UnitsProduct<B, B>>.div(other: A): Measure<UnitsRatio<A, UnitsProduct<B, B>>> =
    numerator.first.ratio / other.ratio * (numerator.second / denominator)

@JvmName("div2")
operator fun <A : Units, B : Units, C : Units> UnitsRatio<UnitsProduct<A, A>, UnitsProduct<B, C>>.div(
    other: A
): Measure<UnitsRatio<A, UnitsProduct<B, C>>> =
    numerator.first.ratio / other.ratio * (numerator.second / denominator)

@JvmName("div3")
operator fun <A : Units, B : Units, C : Units, D : Units> UnitsRatio<UnitsProduct<A, B>, UnitsProduct<C, D>>.div(
    other: A
): Measure<UnitsRatio<B, UnitsProduct<C, D>>> =
    numerator.first.ratio / other.ratio * (numerator.second / denominator)

// m/s * (s^2/m) => s
operator fun <A : Units, B : Units> UnitsRatio<A, B>.div(other: UnitsRatio<A, Square<B>>): Measure<B> =
    numerator.ratio / other.numerator.ratio * (other.denominator / denominator)

// m/s / s => m/s^2
operator fun <A : Units> InverseUnits<A>.div(other: A): Measure<InverseUnits<Square<A>>> =
    ratio * other.ratio * 1 / Square(other, other)
...
@JvmName("times5")
operator fun <A : Units, B : Units, C : Units> Measure<UnitsRatio<A, UnitsProduct<B, C>>>.times(
    other: Measure<B>
): Measure<UnitsRatio<A, C>> = amount * other.amount * (units * other.units)

@JvmName("times6")
operator fun <A : Units, B : Units, C : Units, D : Units> Measure<UnitsRatio<A, UnitsProduct<B, C>>>.times(
    other: Measure<D>
): Measure<UnitsRatio<UnitsProduct<A, D>, UnitsProduct<B, C>>> =
    amount * other.amount * (units * other.units)
...
@JvmName("div12")
operator fun <A : Units, B : Units> Measure<UnitsRatio<UnitsProduct<A, A>, UnitsProduct<B, B>>>.div(
    other: Measure<A>
): Measure<UnitsRatio<A, UnitsProduct<B, B>>> = amount / other.amount * (units / other.units)

@JvmName("div13")
operator fun <A : Units, B : Units, C : Units> Measure<UnitsRatio<UnitsProduct<A, A>, UnitsProduct<B, C>>>.div(
    other: Measure<A>
): Measure<UnitsRatio<A, UnitsProduct<B, C>>> = amount / other.amount * (units / other.units)

@JvmName("div14")
operator fun <A : Units, B : Units, C : Units, D : Units> Measure<UnitsRatio<UnitsProduct<A, B>, UnitsProduct<C, D>>>.div(
    other: Measure<A>
): Measure<UnitsRatio<B, UnitsProduct<C, D>>> = amount / other.amount * (units / other.units)
...
@JvmName("times16")
operator fun <A : Units> Measure<InverseUnits<A>>.times(other: A): Double = amount * (units * other)

@JvmName("times16")
operator fun <A : Units> Measure<A>.times(other: InverseUnits<A>): Double = amount * (units * other)
...
operator fun <A : Units, B : Units, C : Units> Measure<UnitsRatio<A, UnitsProduct<B, C>>>.times(
    other: B
): Measure<UnitsRatio<A, C>> = amount * (units * other)

@JvmName("times6")
operator fun <A : Units, B : Units, C : Units, D : Units> Measure<UnitsRatio<A, UnitsProduct<B, C>>>.times(
    other: D
): Measure<UnitsRatio<UnitsProduct<A, D>, UnitsProduct<B, C>>> = amount * (units * other)
...
@JvmName("div16")
operator fun <A : Units> Measure<A>.div(other: A): Double = amount * (units.ratio / other.ratio)
...
operator fun <A : Units, B : Units> Measure<UnitsRatio<UnitsProduct<A, A>, UnitsProduct<B, B>>>.div(
    other: A
): Measure<UnitsRatio<A, UnitsProduct<B, B>>> = amount * (units / other)

@JvmName("div13")
operator fun <A : Units, B : Units, C : Units> Measure<UnitsRatio<UnitsProduct<A, A>, UnitsProduct<B, C>>>.div(
    other: A
): Measure<UnitsRatio<A, UnitsProduct<B, C>>> = amount * (units / other)

@JvmName("div14")
operator fun <A : Units, B : Units, C : Units, D : Units> Measure<UnitsRatio<UnitsProduct<A, B>, UnitsProduct<C, D>>>.div(
    other: A
): Measure<UnitsRatio<B, UnitsProduct<C, D>>> = amount * (units / other)
...
operator fun <T : Units> Number.div(measure: Measure<T>): Measure<InverseUnits<T>> =
    this.toDouble() / measure.amount * InverseUnits(measure.units)

@JvmName("divInvMeasure")
operator fun <T : Units> Number.div(measure: Measure<InverseUnits<T>>): Measure<T> =
    this * measure.units.unit / measure.amount
